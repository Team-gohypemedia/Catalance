...
export const PmShell = ({ title = "Management", subtitle, actions, children, hideHeader, className }) => {
  return (
    <RoleAwareSidebar>
      <div className={`min-h-dvh overflow-x-clip bg-background text-foreground ${className || ""}`}>
        <ManagerTopBar />
        <div className="mx-auto w-full max-w-[1600px] min-w-0 space-y-5 p-3 pb-3 sm:p-4 sm:pb-4 md:space-y-6 md:p-5 md:pb-5 xl:p-6 xl:pb-6">
          {!hideHeader && (
            <div className="flex flex-wrap items-start justify-between gap-6">
              <div>
                <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-[2.1rem] xl:text-4xl">{title}</h1>
                {subtitle ? <p className="mt-2 max-w-3xl text-sm font-medium text-muted-foreground sm:text-base">{subtitle}</p> : null}
              </div>
