...
const calculatePosition = (value, containerSize, elementSize) => {
...
const parsePathToVertices = (path, sampleLength = 15) => {
  // Stub for SVG parsing since we only use standard bodies
  return [];
};
import {
  Bodies,
...
const Gravity = forwardRef((
  ...
  const mouseConstraint = useRef(undefined)
  ...
  const initializeRenderer = useCallback(() => {
    ...
    // Fix page scrolling by removing matter.js scroll/wheel listeners
    mouse.element.removeEventListener("mousewheel", mouse.mousewheel);
    mouse.element.removeEventListener("DOMMouseScroll", mouse.mousewheel);

    // Also remove touch listeners so mobile scroll isn't blocked 
    mouse.element.removeEventListener("touchmove", mouse.mousemove);
    mouse.element.removeEventListener("touchstart", mouse.mousedown);
    mouse.element.removeEventListener("touchend", mouse.mouseup);

    // Re-bind touch events with a wrapper that only prevents default if dragging a body
    const handleTouch = (fn) => (e) => {
      const originalPreventDefault = e.preventDefault;
      e.preventDefault = () => {
        if (mouseConstraint.current && mouseConstraint.current.body) {
    ...
    mouse.element.addEventListener("touchstart", handleTouch(mouse.mousedown), { passive: false });
    mouse.element.addEventListener("touchmove", handleTouch(mouse.mousemove), { passive: false });
    mouse.element.addEventListener("touchend", handleTouch(mouse.mouseup), { passive: false });
