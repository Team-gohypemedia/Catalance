...
const FreelancerSelectionCard = memo(({
  freelancer: initialFreelancer,
  bestMatchFreelancerIds,
  isSendingProposal,
  sendingFreelancerId,
  setActiveAiFreelancerId,
  onViewFreelancer,
  onSendProposal,
  generateGradient,
}) => {
  const { authFetch } = useAuth();
  const [freelancerData, setFreelancerData] = useState(initialFreelancer || {});

  useEffect(() => {
    const hasData = freelancerData?.budgetFitScore !== undefined || freelancerData?.projectsDelivered !== undefined;
    const freelancerId = initialFreelancer?.freelancerId || initialFreelancer?.id;

    if (!hasData && freelancerId && authFetch) {
      let isMounted = true;
      authFetch(`/users/${freelancerId}`)
        ...
        .then(payload => {
          if (isMounted) {
            ...
            setFreelancerData(prev => ({ ...prev, ...data, id: freelancerId }));
          }
        })
        .catch(err => console.error("Failed to fetch freelancer details", err));
      return () => { isMounted = false; };
    }
  }, [freelancerData?.budgetFitScore, freelancerData?.projectsDelivered, initialFreelancer?.id, initialFreelancer?.freelancerId, authFetch]);

  const freelancer = { ...initialFreelancer, ...freelancerData };
  ...
  const performanceStats = [
    {
      key: "rating",
      label: "Rating",
      value: formatRating(freelancer.rating || freelancer.ratingScore),
      showStar: true,
    },
    {
      key: "budget-fit",
    ...
    },
    {
      ...
      value: deliveredProjectCount !== "N/A" ? deliveredProjectCount : (Number.isFinite(freelancer?.projectsDelivered) ? freelancer.projectsDelivered : "N/A"),
    },
  ];
        ...
        <div className="flex items-start justify-between gap-2 border-b border-border dark:border-white/10 pb-1.5">
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-1">
              ...
              {isVerified && (
                <div
                  className="flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full bg-[#1da1f2]"
            ...
            <div className="mt-0.5 flex flex-wrap gap-1">
              {matchScore !== null && (
                <Badge
                  variant="outline"
                  className="h-4 border-border/80 bg-transparent px-1 text-[9px] whitespace-nowrap"
                >
                  {matchScore}% Match
                </Badge>
              )}
                  ...
                  >
                    {group.freelancers.map((freelancer) => (
                      <FreelancerSelectionCard
                        ...
                        setActiveAiFreelancerId={setActiveAiFreelancerId}
                        onViewFreelancer={onViewFreelancer}
