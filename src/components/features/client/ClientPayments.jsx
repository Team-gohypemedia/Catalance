...
const isCompletedProject = (status = "") => String(status).toUpperCase() === "COMPLETED";

const isAwaitingDeposit = (status = "", spent = 0) =>
  String(status).toUpperCase() === "AWAITING_PAYMENT" || Number(spent || 0) <= 0;
...
const ClientPaymentsContent = () => {
  ...
  const invoices = useMemo(() => {
    return projects
      ...
      .map((project) => {
        ...
        const awaiting = isAwaitingDeposit(status, spent);
        const completed = isCompletedProject(status);
        const escrowHeld = !awaiting && !completed ? spent : 0;
        return {
          id: project.id,
  ...
  const billingSummary = useMemo(() => {
    return invoices.reduce(
      (acc, invoice) => {
        ...
        if (invoice.awaiting) acc.awaitingDeposit += 1;
        return acc;
      },
      { totalPaid: 0, totalEscrow: 0, awaitingDeposit: 0 }
    );
  }, [invoices]);
  ...
  const handleOpenCustomerPortal = async () => {
    ...
    try {
      ...
      toast.info("Customer portal endpoint is not wired yet. Integrate Stripe portal URL to enable it.");
    } catch (error) {
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 z-10 relative scroll-smooth">
        <div className="max-w-[1600px] mx-auto space-y-6">
          <header className="space-y-2">
            <p className="text-sm uppercase tracking-[0.35em] text-primary/70">Escrow and invoices</p>
            <h1 className="text-3xl font-black tracking-tight text-foreground">Billing Hub</h1>
            <p className="text-sm text-muted-foreground">
              Manage payment methods, download receipts, and track funds currently held in escrow.
            </p>
          </header>
          ...
          {isLoading ? (
            ...
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              <BillingMetric
                ...
                hint="Funds locked until milestone approval and payout release."
                icon={Wallet}
              />
              <BillingMetric
                title="Total paid"
                value={formatCurrency(billingSummary.totalPaid)}
                hint="Client deposits and released payments across accepted projects."
                icon={Landmark}
              />
              <BillingMetric
                title="Pending deposits"
                value={String(billingSummary.awaitingDeposit)}
                hint="Projects waiting for upfront funding to begin work."
                icon={ShieldCheck}
              />
            </section>
          )}
          ...
          <section className="grid gap-4 lg:grid-cols-[360px_1fr]">
            <Card className="border-border/60 bg-card/70 backdrop-blur-sm">
              ...
              <CardContent className="space-y-3">
                ...
                <Button
                  ...
                  onClick={() => toast.info("Use the billing portal to add and remove cards securely.")}
                >
            ...
            <Card className="border-border/60 bg-card/70 backdrop-blur-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Download receipts for each funded project and track escrow state.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="rounded-lg border border-dashed border-border/70 bg-muted/20 px-4 py-8 text-center">
                    <p className="text-sm text-muted-foreground">No invoices yet. Fund a project to see receipts.</p>
                  </div>
                ) : (
                    ...
                    {invoices.map((invoice) => (
                      ...
                      >
                        <div className="min-w-[200px]">
                          <p className="text-sm font-semibold text-foreground">{invoice.projectTitle}</p>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Badge variant={invoice.awaiting ? "secondary" : "outline"}>{invoice.statusLabel}</Badge>
                          <Button
                            size="sm"
                            variant="outline"
                            className="gap-2"
