  ...
  return (
    <div className={cn("relative min-h-screen bg-background text-foreground", category !== "all" && "pt-28")}>
      ...
      {category === "all" && (
        ...
        <section className="relative overflow-hidden bg-[#F9F5EE] dark:bg-[#1a1a1a] min-h-[500px] w-full rounded-[10px] shadow-sm mb-12 border border-border">
          <div className="relative z-10 flex flex-col items-center pt-24 text-center px-4 pointer-events-none">
            <Badge className="rounded-md bg-primary dark:bg-primary px-6 py-2.5 text-sm md:text-base font-bold uppercase tracking-[0.2em] !text-white dark:!text-black hover:bg-primary/25 dark:hover:bg-primary/30 shadow-sm border border-primary/20">
              SERVICES WE SERVE
            </Badge>
          ...
          <Gravity gravity={{ x: 0, y: 1 }} grabCursor={true} className="absolute inset-0 z-20 pointer-events-auto">
            {[
              { text: "Web Development", color: "bg-[#3B66FF] text-white", x: "15%", y: "15%", angle: 12 },
              { text: "Graphics Design", color: "bg-[#71C7A5] text-white", x: "30%", y: "25%", angle: -10 },
              { text: "UI/UX Design", color: "bg-[#F8CC72] text-[#2B2B2B]", x: "45%", y: "10%", angle: 5 },
              { text: "Customer Support", color: "bg-[#679CE0] text-white", x: "60%", y: "30%", angle: -8 },
              { text: "Content & Writing", color: "bg-[#FF6A39] text-white", x: "35%", y: "10%", angle: 20 },
              { text: "SEO", color: "bg-[#FCA17B] text-white", x: "55%", y: "20%", angle: -15 },
              { text: "Software Development", color: "bg-[#3B54D5] text-white", x: "75%", y: "15%", angle: 18 },
              { text: "Voice Agent", color: "bg-[#FFA8AF] text-white", x: "85%", y: "30%", angle: -5 },
            ].map((service, idx) => (
              <MatterBody
                key={idx}
                x={service.x}
              ...
              >
                <div className={cn("px-8 py-4 sm:px-10 sm:py-5 rounded-[40px] text-lg sm:text-xl font-medium shadow-md cursor-grab active:cursor-grabbing", service.color)}>
                  {service.text}
                </div>
              </MatterBody>
            ))}
